...
| Learning enabled | Per-zone toggle. When on, Amazing Irrigation learns a physics-informed **Soil Water Balance** (Irrigation Efficiency, Rain Efficiency, an Evapotranspiration coefficient, a Drainage rate, and bounded Field Capacity / Wilting Point — each with a Model Confidence) and uses it for **Predictive Control**, always bounded by safety limits with any manual value winning. Field Capacity is learned as the **drained upper limit** — the settled plateau after a saturation→drainage cycle, not the wetting peak — so clear saturation history corrects a wrong Field Capacity while transient peaks and offline spikes are ignored. When off, observations are still collected but never change decisions. |
